<script lang="ts">
	...
	let modalImgNum = 0;
	let modalImgIndex = 0;
	let modalisReported = writable(false);
	let comments = [];
	let comment = '';
	let sse: EventSource;
	...
	async function showModalFnc(img: components['schemas']['ImageDto'], num, index) {
		...
		const isReported = await rq.apiEndPoints().GET('/api/v1/report/isReport', {
			params: {
				query: {
					imageId: img.id,
		...
		modalisReported.set(isReported.data?.data!);
		console.log($modalisReported);
	...
	async function report() {
		...
		modalisReported.update((value) => !value);
